...
class UserDictionary:
    ...
    def is_protected(self, word: str, lang: str, timeout: float = 10.0) -> bool:
        key = self._key(word, lang)
    ...
    def add_correction(self, word: str, lang: str, debug: bool = False) -> None:
        """Penalise auto-conversion for this word (-1 weight)."""
        key = self._key(word, lang)
        entry = self.data["words"].setdefault(key, {"weight": 0})
        entry["weight"] -= 1
        if debug:
            logger.debug("UserDict correction: %s weight → %d", key, entry["weight"])
        self.flush()
