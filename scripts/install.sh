...
offer_system_dictionaries() {
    ...
    answer="${answer:-y}"
    if [[ ! "$answer" =~ ^[Yy]$ ]]; then
        info "Установка словарей пропущена; системная префиксная конвертация будет недоступна."
        info "Точные решения user_dict и n-gram fallback продолжат работать."
        return 0
    fi
...
install() {
    ...
    # Для безопасной префиксной конвертации нужны оба языка; установка optional.
    offer_system_dictionaries
