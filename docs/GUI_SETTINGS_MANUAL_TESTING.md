...
8. На каждой странице навести указатель на значки `ⓘ` у настроек.
   Ожидается: появляются локализованные описания без технических ключей;
   описание соответствует назначению параметра, использует уменьшенный шрифт
   и переносится внутри поля шириной `260 px`.
9. Отключить родительскую функцию и навести указатель на `ⓘ` отключённого
   дочернего параметра.
   Ожидается: control отключён, но его описание по-прежнему доступно.
