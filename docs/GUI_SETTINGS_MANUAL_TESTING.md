...
## Видимость настроек платформы

1. В X11 открыть страницу «Выделение».
   Ожидается: видны X11 selection timings; Wayland strategy и timings скрыты.
2. В Wayland открыть страницу «Выделение».
   Ожидается: видны Wayland strategy и timings; X11 selection timings скрыты.
3. Изменить общий параметр и применить его, затем проверить `config.toml`.
   Ожидается: скрытые значения другой платформы не изменились.
4. Открыть диалог в тестовом окружении без определяемого X11/Wayland session
   type.
   Ожидается: видны обе платформенные группы и предупреждение, что тип
   окружения не определён и LSwitch может работать некорректно.
