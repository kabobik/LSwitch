...
### 3.3. PyQt5 мешает Wayland/KDE backend

UI импортирует PyQt5, а KDE-native D-Bus планируется через QtDBus.

Решение:

- мигрировать GUI на PyQt6;
- заменить imports и enum scopes;
- `QAction` брать из `PyQt6.QtGui`;
- `exec_()` заменить на `exec()`;
- обновить `setup.py`, `requirements.txt`, README.
...
- Перенести UI imports с PyQt5 на PyQt6.
- Исправить enum scopes.
- Перенести `QAction` в `QtGui`.
- Заменить `exec_()` на `exec()`.
- Обновить packaging.
