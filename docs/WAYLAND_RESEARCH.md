...
> но прежние вызовы с signatures `i`, `s`, `ss` не подходят. Поэтому реализация
> должна диагностировать реальные methods/signatures через D-Bus introspection и
> иметь fallback через `switchToNextLayout`.
