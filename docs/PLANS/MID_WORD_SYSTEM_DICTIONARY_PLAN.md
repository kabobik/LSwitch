...
> Архивный план. Отдельный `auto_switch_mid_word` и встроенные мини-словари
> удалены. Актуальная архитектура описана в
...
mid-word режим optional. Реализован следующий сценарий:

- installer ищет EN и RU `.dic` в тех же Hunspell/MySpell-каталогах и с тем же
  приоритетом имён, что `SystemDictionaryLoader`;
- если оба языка найдены, installer показывает выбранные пути и продолжает;
- если отсутствует один или оба языка, пользователь получает отдельное
  предложение установки `[Y/n]`;
- через `apt` устанавливаются `hunspell-en-us` / `hunspell-ru`, через `pacman` —
  `hunspell-en_us` / `hunspell-ru`, причём уже найденный язык не запрашивается;
- отказ не прерывает установку LSwitch: остаются доступны встроенные словари;
- ошибка пакетного менеджера при опциональной установке словарей выводится как
  предупреждение и также не прерывает основную установку;
- после установки пакетов пути EN/RU проверяются повторно, отсутствие файла
  выводится как предупреждение, а не фатальная ошибка;
- diagnostics показывают наличие EN/RU системных словарей и word counts.
...
- [x] Предлагать опциональную установку недостающих EN/RU словарей в
  `scripts/install.sh`.
