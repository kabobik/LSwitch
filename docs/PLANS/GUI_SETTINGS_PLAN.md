...
значения сохраняются сразу и будут переданы adapter-у при следующем запуске в
...
- пометка, какая секция относится к текущей сессии. Секцию другой платформы
  не скрывать: пользователь может заранее настроить ее.
