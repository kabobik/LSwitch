...
> Примечание 2026-07-13: описания `auto_switch_mid_word` и
> `user_dict_auto_confirm` ниже сохранены как история первоначальной реализации.
> Актуальный единый pipeline и удаление этих параметров зафиксированы в
...
### 10.6 Контекстные описания

Каждая строка изменяемой настройки содержит компактный значок `ⓘ`. При
наведении он показывает локализованное описание фактической семантики
параметра: назначение, существенные последствия и специальные значения вроде
`0` или пустого пути. Tooltip использует немного уменьшенный шрифт, контентную
ширину `260 px` и перенос строк. Для очевидных read-only строк, например пути
к текущему `config.toml`, отдельный значок не нужен. Значок остаётся доступным,
когда дочерний control отключён зависимостью, и скрывается вместе со всей
строкой неактивной платформы.

## 11. Визуальные зависимости

Зависимость управляет `enabled` state controls, label и browse button как
единой группы. Контекстный значок `ⓘ` остаётся активным, чтобы описание можно
было прочитать и для недоступного дочернего параметра. Значение disabled
control не сбрасывается.
...
- наличие локализованного help text для каждого settings binding;
- видимость help icon следует platform visibility, но не dependency enabled;
...
- [x] Все изменяемые настройки имеют локализованное всплывающее описание.
