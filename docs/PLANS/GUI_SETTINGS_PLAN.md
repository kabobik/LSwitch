...
`0` или пустого пути. Для очевидных read-only строк, например пути к текущему
`config.toml`, отдельный значок не нужен. Значок остаётся доступным, когда
дочерний control отключён зависимостью, и скрывается вместе со всей строкой
неактивной платформы.
