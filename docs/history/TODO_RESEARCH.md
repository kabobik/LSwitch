...
## Логирование и буферизация событий (Анализ логов 1_2_3)
- [ ] **Анализ лога автоконвертации:** Пробел успевает напечататься и удаляется. Добавить расширенное логирование всех этапов `_do_auto_conversion_at_space` (backspace, switch, replay, space) для точного понимания таймингов.
- [ ] **Исследование перехвата ввода во время retype:** Когда идет конвертация (~65мс с sleep), пользователь может продолжать печать. События от evdev "просачиваются" либо удаляются бэкспейсами, либо ломают replay. Нужно исследовать возможность "накопления" (буферизации) нажатий в event_buffer во время конвертации, и отправки их после её завершения, ИЛИ грабать устройство (EVIOCGRAB), чтобы X11 не получал их до конца конвертации.
