# LSwitch - Быстрый переключатель раскладки клавиатуры

Перехватчик событий клавиатуры на уровне ядра (evdev) для быстрого преобразования текста между английской и русской раскладками через двойное нажатие Shift.

## Возможности

- ✅ **Конвертация текста по двойному Shift** — нажми Shift дважды быстро, и текст конвертируется
- ✅ **Работает везде** — X11, консоль (tty), все приложения
- ✅ **Перехват на уровне ядра** — через `/dev/input/`, максимальная скорость
- ✅ **Конвертация слова или выделения** — последнее слово ИЛИ выделенный пользователем текст
- ✅ **EN ⟷ RU** — английский ↔ русский
- ✅ **GUI панель управления** — KDE Plasma, Linux Mint Cinnamon
- ✅ **systemd демон** — автозапуск при входе в систему
- ✅ **Самообучающийся словарь** — учится на истории ошибок

## Установка

### Требования

- Linux (любой дистрибутив с поддержкой evdev)
- Python 3.8+
- pip (обычно идёт вместе с Python)

### Шаг 1: Установите системные зависимости

```bash
# Ubuntu/Debian
sudo apt install python3-pip python3-dev libxclip0 xdotool

# Fedora
sudo dnf install python3-pip python3-devel xclip xdotool

# Arch
sudo pacman -S python3 xclip xdotool
```

### Шаг 2: Клонируйте и установите LSwitch

```bash
git clone https://github.com/yourusername/lswitch.git
cd lswitch

# Установка (потребуются права root для доступа к устройствам ввода)
sudo pip3 install -e .
```

### Шаг 3: Настройте права доступа (ВАЖНО!)

⚠️ **По умолчанию** `/dev/input/` доступны только root. Без настройки прав программа будет требовать sudo.

**Решение:** Добавьтесь в группу `input`:

```bash
# Добавить текущего пользователя в группу input
sudo usermod -a -G input $USER

# Создать udev правило (опционально, для надёжности)
sudo tee /etc/udev/rules.d/99-lswitch.rules > /dev/null <<'EOF'
KERNEL=="event*", SUBSYSTEM=="input", MODE="0660", GROUP="input"
EOF

sudo udevadm control --reload-rules
sudo udevadm trigger
```

**⚠️ ВАЖНО:** После этого **выйдите и войдите снова** (или перезагрузитесь) для применения прав.

Проверка:
```bash
groups $USER
# Должно содержать "input"
```

## Использование

### Запуск GUI (рекомендуется)

После установки запустите:

```bash
lswitch-control
```

Или найдите в меню приложений: **Приложения → LSwitch Control**

**Возможности GUI:**
- Включение/выключение сервиса
- Настройка параметров
- Просмотр логов
- Информация о системе

### Запуск демона (systemd)

```bash
# Запустить
sudo systemctl start lswitch

# Включить автозапуск
sudo systemctl enable lswitch

# Проверить статус
sudo systemctl status lswitch

# Просмотр логов
sudo journalctl -u lswitch -f
```

### Запуск вручную (для отладки)

```bash
# Требует sudo для доступа к /dev/input/
sudo python3 -m lswitch

# Или с отладкой
sudo python3 -m lswitch --debug
```

## Как это работает

1. **Перехват события:** Программа слушает событие "нажимание Shift" через `/dev/input/`
2. **Детектор двойного клика:** Ждёт второго нажатия Shift в течение 0.3 сек
3. **Получение текста:** Извлекает последнее слово (или выделенный текст) из буфера обмена
4. **Конвертация:** Преобразует текст через словарь (EN ↔ RU)
5. **Вставка:** Удаляет оригинальный текст (Backspace) и вставляет конвертированный (Ctrl+V)
6. **Переключение раскладки:** Автоматически переключает раскладку (Alt+Shift)

## Примеры

| Введи | Двойной Shift | Результат |
|------|---|---|
| `ghbdtn` | → | `привет` (+ раскладка на RU) |
| `привет` | → | `ghbdtn` (+ раскладка на EN) |
| `Hello world` (выделено) | → | `Привет мир` |

## Конфигурация

Конфиг находится в `~/.config/lswitch/config.json` (или `/etc/lswitch/config.json` если установлено как systemd сервис):

```json
{
  "double_shift_timeout": 0.3,
  "switch_layout_after_convert": true,
  "repair_enabled": false,
  "debug": false,
  "layouts": ["en", "ru"],
  "conversion_rules": "en->ru",
  "keyboard_layout_switch_key": "Alt_L+Shift_L"
}
```

**Параметры:**
- `double_shift_timeout` — максимальное время между двумя нажатиями Shift (сек)
- `switch_layout_after_convert` — автоматически переключать раскладку после конвертации
- `repair_enabled` — включить попытки восстановления при ошибках (обычно `false`)
- `debug` — выводить отладочные сообщения
- `keyboard_layout_switch_key` — комбинация для переключения раскладки (Alt+Shift, Ctrl+Shift, Win+Space)

После изменения конфига перезапустите сервис:
```bash
sudo systemctl restart lswitch
```

## Архитектура

```
lswitch/
├── core.py              # Основной цикл обработки событий
├── cli.py               # Интерфейс командной строки
│
├── adapters/            # Адаптеры для DE
│   ├── x11.py          # X11 clipboard/selection
│   ├── kde.py          # KDE Plasma GUI
│   ├── cinnamon.py     # Linux Mint Cinnamon GUI
│   └── base.py         # Базовый класс
│
├── utils/              # Утилиты
│   ├── keyboard.py     # evdev, виртуальная клавиатура
│   ├── buffer.py       # Буфер последних слов
│   └── desktop.py      # Определение DE
│
├── conversion.py       # Движок конвертации (EN ↔ RU)
├── selection.py        # Работа с выделениями
├── dictionary.py       # Словарь слов
├── ngrams.py          # N-грамм анализ для автоисправления
└── user_dictionary.py # Пользовательский словарь
```

Подробнее: [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)

## Тестирование

```bash
# Запуск тестов
pytest -v

# Запуск конкретного теста
pytest tests/test_conversion.py -v

# С покрытием
pytest --cov=lswitch tests/
```

**Статус:** 98 тестов, все проходят ✅

## Решение проблем

**Программа требует sudo даже после установки:**
- Проверьте, что вы в группе `input`: `groups $USER`
- Если нет — добавьте и перелогиньтесь: `sudo usermod -a -G input $USER`

**Конвертация не работает:**
- Включите отладку: отредактируйте config.json, установите `"debug": true`
- Посмотрите логи: `sudo journalctl -u lswitch -f`
- Проверьте раскладку: `setxkbmap -query`

**GUI не запускается:**
- Установите PyQt5: `sudo pip3 install PyQt5`
- Проверьте DE: `echo $XDG_CURRENT_DESKTOP`

**Нет доступа к /dev/input/:**
```bash
# Проверка
ls -la /dev/input/event* | head -1
# Должно быть что-то с группой "input"

# Если нет — создать правило:
sudo tee /etc/udev/rules.d/99-lswitch.rules > /dev/null <<'EOF'
KERNEL=="event*", SUBSYSTEM=="input", MODE="0660", GROUP="input"
EOF
```

Подробнее: [docs/TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md)

## Документация

- 🚀 [Быстрый старт](docs/QUICKSTART.md) — начало за 5 минут
- 📖 [Подробная установка](docs/INSTALL.md) — детальные шаги
- 🏗️ [Архитектура](docs/ARCHITECTURE.md) — техническое устройство
- 💡 [Примеры](docs/EXAMPLES.md) — различные сценарии использования
- 🔧 [Права доступа](docs/PERMISSIONS.md) — настройка evdev
- 🚀 [Деплой](docs/DEPLOYMENT.md) — развёртывание на серверах
- 🐛 [Решение проблем](docs/TROUBLESHOOTING.md) — FAQ
- 📝 [История изменений](docs/CHANGELOG.md) — что нового

## Разработка

### Локальный запуск

```bash
git clone https://github.com/yourusername/lswitch.git
cd lswitch

# Установить зависимости
pip install -e .
pip install pytest pytest-cov

# Настроить PYTHONPATH
export PYTHONPATH=/path/to/lswitch:$PYTHONPATH

# Запустить тесты
pytest -v

# Запустить приложение в отладке
sudo PYTHONPATH=. python3 -m lswitch --debug
```

### Структура кода

- `lswitch/core.py` — основная логика обработки
- `lswitch/cli.py` — CLI интерфейс
- `adapters/` — интеграция с DE (KDE, Cinnamon)
- `tests/` — модульные и интеграционные тесты
- `utils/` — вспомогательные функции

### Добавление новой раскладки

Отредактируйте `conversion.py` и `ngrams.py`, добавив новую пару конвертации.

## Лицензия

MIT License — см. [LICENSE](LICENSE)

## Автор

Anton — 2024

## Благодарности

- Вдохновлено [Punto Switcher](https://yandex.ru/support/punto/) от Яндекса
- Использует [python-evdev](https://python-evdev.readthedocs.io/) для перехвата клавиатуры
- PyQt5 для GUI
