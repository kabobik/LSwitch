...
├── lswitch_control.py      # Панель управления (legacy, архивировано)
# (Старый системный трей был удалён и перемещён в archive/removed_tray)
...
### 1. GUI режим с иконкой в трее (legacy)

Внимание: старый GUI с иконкой в трее был удалён и перемещён в `archive/removed_tray`.
Если вам нужен старый тестовый трей — его можно найти в архиве, но он больше не поддерживается и не устанавливается автоматически.
...
**Примечание:** GUI-версия (трей) больше не поддерживается и удалена из инсталляции. Смотрите `archive/removed_tray` для старой реализации.
